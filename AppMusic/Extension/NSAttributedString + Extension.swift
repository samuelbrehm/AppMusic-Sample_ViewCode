...
extension NSAttributedString {
    public static func featureText(_ likeCount: String, _ duration: String) -> NSAttributedString {
        ...
        image1.image = img1
        image1.bounds = CGRect(x: 0, y: Int((font1.capHeight - 13).rounded()) / 2, width: 13, height: 13)
        image1.setImageHeight(height: 13)
        let img1String = NSAttributedString(attachment: image1)
        attributedText.append(img1String)
        
        attributedText.append(NSAttributedString(string: " \(likeCount) . ",
        ...
        let img2 = UIImage(named: "timeplus")?.withRenderingMode(.alwaysTemplate).withTintColor(.white)
        let image2 = NSTextAttachment()
        image2.image = img2
        image2.bounds = CGRect(x: 0, y: Int((font2.capHeight - 13).rounded()) / 2, width: 13, height: 13)
        let img2String = NSAttributedString(attachment: image2)
        attributedText.append(img2String)
        
        attributedText.append(NSAttributedString(string: " \(duration)",
